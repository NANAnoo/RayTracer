...
vector<Hitable*> box() {
	...
	data = stbi_load("../resource/redstone_lamp.png", &width, &height, &nrChannels, 0);//加载图片
	objs.push_back(
		//Light_white
...
void get_color_line(World world, Camera cam, vector<vector<Color> > &Mat, int WIDTH, int HEIGHT, int snns, int depth, int s,int c) {
...
void Get_color(World &world, Camera &cam, vector<vector<Color> > &Mat, int WIDTH, int HEIGHT, int snns, int depth, int core) {
	vector<future<void> > Threads(core);
	for (int t = 0; t < core;t++) {
		get_color_line(world, cam, Mat, WIDTH, HEIGHT, snns, depth, t, core);
	}
...
int main()
{
	...
	//初始化世界（被观察对象均在此
	World world(box(), false);
	world.set_sky(BLUE_SKY);
	Camera cam;
	int snns = 10;//每个像素计算次数（曝光次数
	int depth = 10;
	cam.LookAt(Vec3(0,0,4), Vec3(0,0,0), Vec3(0, 1, 0), 40.0, (float)WIDTH / (float)HEIGHT);
	Get_color(world, cam, mat, WIDTH, HEIGHT, snns, depth, max_thread);

	Image img(mat);
	img.save("../result/demo.bmp");
	return 0;
}
